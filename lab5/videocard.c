...
static bool rendering_buffer = 0;
static char *current_buffer; /* Pointer to the currently active buffer (for drawing) */
static unsigned int vram_size; /* Total size of VRAM in bytes */
...
void* (vg_init)(uint16_t mode) {
    ...
    memset(video_mem, 0, vram_size); // Clear VRAM
...
int (vg_flip)(){
    rendering_buffer = !rendering_buffer; // Toggle buffer flag
    current_buffer = rendering_buffer ? video_mem : video_mem + vram_size / 2; // Switch to front buffer
    ...
    reg.cx = 0; // X offset
    reg.dx = rendering_buffer ? vmi.YResolution : 0; // Y offset
